...
interface UseRealtimeOptions {
  ...
  onResponseDelta?: (itemId: string, text: string) => void;
  onResponse?: (itemId: string, text: string) => void;
  ...
  const handleDataChannelMessage = async (event: MessageEvent) => {
    ...
    switch (message.type) {
      case "response.audio_transcript.delta": {
        if (onResponseDelta) {
          onResponseDelta(message.item_id as string, message.delta as string);
        }
      ...
      case "response.audio_transcript.done": {
        if (onResponse) {
          onResponse(message.item_id as string, message.transcript as string);
        }
